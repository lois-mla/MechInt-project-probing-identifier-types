...
def evaluate_first_token_accuracy(model, tokenizer, data_path: str):
    ...
    for item in data:
        ...
        # 2. Tokenize prompt and push to the device your model is on
        input_ids = tokenizer.encode(prompt, return_tensors="pt").to(model.cfg.device)

        # 3. Tokenize the correct target string to find the exact target token ID
        # add_special_tokens=False prevents tokenizer from prepending a BOS token
        target_text = item["correct"].strip()
        target_tokens = tokenizer.encode(target_text, add_special_tokens=False)
        
        if not target_tokens:
            total_predictions -= 1  # Skip if the target is empty
            continue
        ...
        with torch.no_grad():
            # TransformerLens returns logits of shape [batch, seq_len, vocab_size]
            logits = model(input_ids)
            
            # Extract predictions generated by the last token in the prompt sequence
            next_token_logits = logits[0, -1, :]
            
            # 5. Get the highest probability token ID
